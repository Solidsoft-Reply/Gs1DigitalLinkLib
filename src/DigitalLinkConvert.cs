...
public static partial class DigitalLinkConvert {
    ...
    /// <param name="compressOtherKeyValuePairs">If true, non-GS1 key-value pairs will be compressed.</param>
    ...
    private static string DoDecompressGs1DigitalLink(
        ...
        bool compressOtherKeyValuePairs,
        string methodName,
        string paramName,
        bool useShortNames = false) {
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    private static string DoCompressGs1DigitalLink(
        ...
        var compressedDL = compressionLevel == CompressionLevel.PartiallyCompressed
            ...
            : BuildCompressedGS1digitalLink(gs1AIs ?? [], uriStem, useOptimisations, nonGs1KeyValuePairs, compressOtherKeyValuePairs);
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            var compressedPart = CompressGs1AIsToBinary(
                separated["nonID"],
                useOptimisations,
                compressOtherKeyValuePairs ? nonGs1KeyValuePairs : []).BinaryToSafe64();
    ...
    // new method that converts a flat associative array of GS1 Application Identifiers and their values into a more structured object in which the primary identification key, key Qualifiers, data attributes and other key=requiredAi parameters from the URI string are clearly identified as such.
    ...
    private static string BuildCompressedGS1digitalLink(
        ...
        var path = "/" + CompressGs1AIsToBinary(
            gs1AIs,
            useOptimisations,
            compressOtherKeyValuePairs ? nonGs1KeyValuePairs : new Dictionary<string, string>()).BinaryToSafe64();

        return uriStem + path + queryString;
    }

    ////////private Dictionary<string, List<Dictionary<string, string>>> DecompressGS1DigitalLinkToStructuredArray(string compressedDigitalLinkUri) {
    ////////    var extractedData = ExtractFromCompressedGs1DigitalLink(compressedDigitalLinkUri);
    ////////    var gs1Data = (Dictionary<string, string>)extractedData["GS1"];
    ////////    var nonGs1KeyValuePairs = (Dictionary<string, string>)extractedData["other"];
    ////////    var structuredArray = BuildStructuredArray(gs1Data, nonGs1KeyValuePairs);
